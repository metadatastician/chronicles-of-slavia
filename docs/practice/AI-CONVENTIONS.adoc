...
- Code / config / scripts: `SPDX-License-Identifier: AGPL-3.0-or-later`.
- Prose docs (`*.adoc`, `*.md` narrative): `SPDX-License-Identifier: CC-BY-SA-4.0`.
- GitHub shows only one repo licence: the root `LICENSE` is AGPL-3.0-or-later
  (so the sidebar reads AGPL-3.0); the canonical texts live in `LICENSES/`
  and prose is CC-BY-SA-4.0 by its
  per-file header, not a second root licence file.
- The estate default denies AGPL; **this repo overrides it** per ADR-0008
  (`docs/decisions/0008-licence-position.adoc`), reaffirmed by the owner
  2026-08-07. AGPL-3.0-or-later is correct here — do not "correct" it back.
- Incorporated MPL-2.0 components keep MPL-2.0. That is legal and intended:
  MPL §3.3 (Secondary Licenses) names AGPL-3.0 explicitly. Bundled PT Serif
  fonts keep OFL-1.1.
